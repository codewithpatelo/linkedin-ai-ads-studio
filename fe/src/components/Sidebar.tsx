...
import { X, Eye, EyeOff, Terminal, Sparkles, Copy, FileText } from 'lucide-react';
          ...
          <div className="p-4 border-b">
            ...
            <div className="space-y-2">
              {steps.map((step, index) => {
                ...
                const isCompleted = currentStepIndex > index || (!isGenerating && currentStepIndex === index);
            ...
            >
              ...
              {isGenerating && (
                <div className="text-yellow-400 animate-pulse">
                  <span className="text-gray-500">[{new Date().toLocaleTimeString()}]</span> {progressMessage}
                </div>
              )}
            </div>
          </div>
            ...
            <div className="p-4 border-b">
              <div className="flex items-center justify-between mb-3">
                ...
                <Button
                  ...
                  className="text-xs"
                >
                  {showPrompts ? <EyeOff className="w-3 h-3" /> : <Eye className="w-3 h-3" />}
                ...
                <ScrollArea className="h-32">
                  <div className="space-y-2">
                    {enhancedPrompts.map((prompt, index) => {
                      const style = ['professional', 'modern', 'creative', 'minimalist', 'bold'][index];
                      return (
                        <Card key={index} className="p-2">
                          <div className="flex items-center gap-2 mb-1">
                            <Badge 
                              variant="outline" 
                              className={`text-xs ${styleColors[style as keyof typeof styleColors]}`}
                            >
                              {style}
                            </Badge>
                          </div>
          ...
          {adCopy && (
            <div className="p-4">
              ...
              <Card className="p-3">
                <div className="space-y-2">
                  <div>
                    <Badge variant="outline" className="text-xs mb-1">Headline</Badge>
                    <p className="text-sm font-medium">{adCopy.headline}</p>
                  </div>
                  <div>
                    <Badge variant="outline" className="text-xs mb-1">Description</Badge>
                    <p className="text-xs text-gray-600">{adCopy.description}</p>
                  </div>
                </div>
              </Card>
            </div>
