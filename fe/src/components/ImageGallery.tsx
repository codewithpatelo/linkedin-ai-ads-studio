...
const LinkedInAdCard = ({
  ...
  const headline =
    adCopy?.headline || `Discover ${companyData?.productName || "Our Product"}`;
  const description =
    adCopy?.description ||
    `${companyData?.businessValue || "Amazing value"} for ${
      companyData?.audience || "our customers"
    }`;
        ...
        <div className="px-4 py-3">
          ...
          <div className="flex items-center justify-between pt-3 border-t border-gray-100">
            <Button
              variant="ghost"
              size="sm"
              className="flex-1 flex items-center justify-center gap-2 text-gray-600 hover:bg-gray-50 py-2"
            >
...
export const ImageGallery = ({
  ...
  const [modificationPrompts, setModificationPrompts] = useState<
          ...
          {images.map((image, index) => (
            ...
            >
              <LinkedInAdCard
                ...
                onModificationChange={(prompt) =>
                  handleModificationChange(image.id, prompt)
                }
        ...
        <Card className="bg-gradient-card shadow-card border-0 mt-8">
          <CardContent className="p-6">
            ...
            <div className="grid md:grid-cols-2 gap-4 text-sm text-muted-foreground">
              <div>
                <strong className="text-foreground">Style changes:</strong>{" "}
                "make it minimalist", "corporate look", "modern design"
              </div>
              <div>
                <strong className="text-foreground">Colors:</strong> "use blue
                theme", "darker colors", "vibrant palette"
              </div>
              <div>
                <strong className="text-foreground">Elements:</strong> "add tech
                icons", "include charts", "professional photo"
              </div>
              <div>
                <strong className="text-foreground">Layout:</strong> "center the
                text", "split layout", "full background"
              </div>
            </div>
          </CardContent>
        </Card>
